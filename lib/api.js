...
const GooglePlay = function GooglePlay (opts) {
  ...
  const login = Promise.method(function (force) {
    ...
    return _request.postAsync({url: _opts.loginUrl, gzip: true, json: false, form: payload})
      .spread(function (res, body) {
        ...
        _opts.authToken = response.auth || response.token;
        return response.auth || response.token;
      });
  });
